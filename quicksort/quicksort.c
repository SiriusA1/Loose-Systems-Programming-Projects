...
static void quicksort_helper(void *array, int left, int right, size_t elem_sz,
                             int (*comp) (const void*, const void*), int iter);
...
static void swap(void *a, void *b, size_t size) {
	char *tempa = (char *)a;
	char *tempb = (char *)b;
	char *tempc = calloc(1, sizeof(*tempa)+size-1);
	memcpy(tempc, tempa, sizeof(*tempa)+size-1);
	char *swap = tempc;
...
static int lomuto(void *array, int left, int right, size_t elem_sz,
	...
	if(right <= left) {
		...
		for(int i = left+1; i <= right; ++i) {
			if(comp(pivot, iptr) > 0) {
				sptr = sptr + 1;
				s++;
				swap((void *)*sptr, (void *)*iptr, elem_sz);
			}
			iptr = iptr + 1;
		}
		swap((void *)*pivot, (void *)*sptr, elem_sz);
		return s;
	}
}
...
static void quicksort_helper(void *array, int left, int right, size_t elem_sz,
	...
	if(!array) {
		...
		iter = iter+1;
		quicksort_helper(array, left, s, elem_sz, comp, iter);
		quicksort_helper(array, s+1, right, elem_sz, comp, iter);
	}
}
...
void quicksort(void *array, size_t len, size_t elem_sz,
               int (*comp) (const void*, const void*)) {
	quicksort_helper(array, 0, len-1, elem_sz, comp, 0);
}
